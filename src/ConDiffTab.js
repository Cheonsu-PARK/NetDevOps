...
class ConDiffTab extends Component {
  ...
  runicmp = async (id, VserverVip, Vserver) => {
    try {
      const res = await axios.post('http://localhost:5001/run/icmp', {
        target: VserverVip,
      });
    ...
    if (this.state.vendorRadio_adc == "A10"){
      this.setState((prevState) => ({
        adcbindSearched: {...prevState.adcbindSearched,
          [id]: {
            ...prevState.adcbindSearched[id],
            ["icmp_result"]: result, // Save the result of the ICMP
          },
        },
      }));
    }

   else if (this.state.vendorRadio_adc == "Citrix"){
    console.log("citrix in")
      this.setState((prevState) => ({
        adcbindSearched: {...prevState.adcbindSearched,
          [Vserver]: {
            ...prevState.adcbindSearched[Vserver],
            ["icmp_result"]: result, // Save the result of the ICMP
          },
        },
      }));
      ...
      // console.log(this.state.adcbindSearched);
    } catch (error) {
      console.error('Failed to get ping:', error);
    }
  };

  handleICMPButtonClick = (id, VserverVip, Vserver) => {
    this.runicmp(id, VserverVip, Vserver);
  };

  renderTableData_adc = () => {
    const { adcbindSearched, icmpresult} = this.state;
    let datarow= [];
    // let dastacell = [];
    let Columns =[]
    ...
    let Columns_a10 = [
      ...
      { field: 'Active-Partion', headerName: 'Active-Partion', width: 130 },
      { field: 'VserverName', headerName: 'VserverName', width: 300 },
      { field: 'VserverVip', headerName: 'VserverVip', width: 130 },
      { field: 'VserverPort', headerName: 'VserverPort', width: 130 },
      { field: 'service-group', headerName: 'service-group', width: 200 },
      { field: 'member', headerName: 'member', width: 450,
      renderCell: (params) => (
        <div>
      ...
      { field: 'icmp_result', headerName: 'icmp_result', width: 300 },
    ]
      Columns = Columns_a10
    ...
    Columns = Columns_citrix
  }
    const tableRows = Object.keys(adcbindSearched).map((vserver) => {
      const vserverData = adcbindSearched[vserver];
      datarow.push(vserverData)

      // datacell = Object.keys(vserverData)
      const icmpResult = icmpresult[vserver];
    ...
    return(
      ...
      <DataGrid rows={datarow}
                ...
                getRowHeight={() => 'auto'}
                onCellClick={(params) => {
                  if (params.field === 'icmp_result') {
                    const { id, VserverVip, VserverName } = params.row;
                    this.runicmp(id, VserverVip, VserverName);
                    console.log(this.state.adcbindSearched)
                    console.log(id)
                    console.log(VserverVip)
                    console.log(VserverName)
                  }
                }}
                // getCellClassName={  (params) => {
                  // return params.field === 'columnName' ? 'cell-line-break' : '';
                // }}
                >
